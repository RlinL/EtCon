...
class FSDPWorker(Worker):
    ...
    def _build_model_optimizer(
        ...
        # Handle trainable layers configuration (both legacy and fine-grained)
        if (model_config.trainable_layers is not None or model_config.trainable_components is not None) and (self._is_actor or self._is_critic):
            # First freeze all parameters
            model.requires_grad_(False)
            
            # Find layers container
            layer_attr_names = ['layers', 'transformer.h', 'model.layers', 'decoder.layers', 'encoder.layer']
            layers_container = None
            
            for attr_name in layer_attr_names:
                if hasattr(model, attr_name):
                    layers_container = getattr(model, attr_name)
                    break
            ...
            if layers_container is not None:
                ...
                # Handle fine-grained component control
                if model_config.trainable_components is not None:
                    valid_layers = []
                    valid_components = []
                    
                    for layer_idx in model_config.trainable_components.layers:
                        if 0 <= layer_idx < total_layers:
                            layer = layers_container[layer_idx]
                            valid_layers.append(layer_idx)
                            
                            # Set specific components trainable (supporting nested components)
                            for component_name in model_config.trainable_components.components:
                                # Handle nested components (e.g., "mlp.down_proj", "self_attn.q_proj")
                                if '.' in component_name:
                                    parts = component_name.split('.')
                                    current_module = layer
                                    component_path = f"layer_{layer_idx}"
                                    ...
                                    try:
                                        for part in parts:
                                            if hasattr(current_module, part):
                                                ...
                                                component_path += f".{part}"
                                            else:
                                                raise AttributeError(f"'{part}' not found")
                                        
                                        # Set the final nested component trainable
                                        current_module.requires_grad_(True)
                                        valid_components.append(component_path)
                                        self.print_rank0(f"Set nested component {component_name} in layer {layer_idx} to trainable")
                                        
                                    except AttributeError as e:
                                        self.print_rank0(f"Warning: Nested component {component_name} not found in layer {layer_idx}: {e}")
                                
                                # Handle top-level components  
                                elif hasattr(layer, component_name):
                                    component = getattr(layer, component_name)
                                    component.requires_grad_(True)
                                    valid_components.append(f"layer_{layer_idx}.{component_name}")
                                    self.print_rank0(f"Set {component_name} in layer {layer_idx} to trainable")
                                else:
                                    self.print_rank0(f"Warning: Component {component_name} not found in layer {layer_idx}")
                        else:
                            self.print_rank0(f"Warning: Layer {layer_idx} is out of range (0-{total_layers-1}), skipping.")
                    
                    if valid_components:
                        fsdp_config.use_orig_params = True
                        self.print_rank0(f"Fine-grained control: {len(valid_components)} components set to trainable:")
                        for comp in valid_components:
                            self.print_rank0(f"  - {comp}")
                    else:
                        self.print_rank0("No valid trainable components specified, all parameters remain frozen.")
                
                # Handle legacy trainable_layers (for backward compatibility)
                elif model_config.trainable_layers is not None:
                    valid_layers = []
                    
                    for layer_idx in model_config.trainable_layers:
                        if 0 <= layer_idx < total_layers:
                            layers_container[layer_idx].requires_grad_(True)
                            valid_layers.append(layer_idx)
                        else:
                            self.print_rank0(f"Warning: Layer {layer_idx} is out of range (0-{total_layers-1}), skipping.")
                    ...
                    if valid_layers:
                        fsdp_config.use_orig_params = True
                        self.print_rank0(f"Legacy mode: layers {valid_layers} are set to trainable out of {total_layers} total layers.")
                    else:
                        self.print_rank0("No valid trainable layers specified, all parameters remain frozen.")
            else:
                self.print_rank0("Warning: Could not find model layers container, trainable configuration ignored.")
